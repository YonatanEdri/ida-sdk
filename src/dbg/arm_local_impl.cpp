...
ssize_t idaapi dbg_listener_t::on_event(ssize_t code, va_list)
{
  switch ( code )
  {
    ...
    case dbg_process_attach:
      // For 64-bit ARM targets (AArch64), enable hardware single-step support.
      // 32-bit ARM has no hardware single-step; it uses software breakpoints.
      if ( inf_is_64bit() )
      {
        pm.debugger.flags |= DBG_HAS_SET_RESUME_MODE;
        pm.debugger.resume_modes |= DBG_RESMOD_STEP_INTO;
      }
      break;
