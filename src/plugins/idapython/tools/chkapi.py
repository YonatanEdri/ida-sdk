"""Checks python wrapper files contain sensitive correct wrappers"""
...
def check_cpp(args):
    ...
    functions_coherence = {
        ...
        "_wrap_load_debugger" : {
            "string" : ["SWIG_PYTHON_THREAD_BEGIN_ALLOW",
        ...
        "_wrap_vdui_t_cfunc_get" : {
            "mustcall" : "hexrays_register_python_clearable_instance",
            # proper typemap must be used
            "string" : "SWIGTYPE_p_qrefcnt_tT_cfunc_t_t",
        },
        "delete_cexpr_t" : {
            "mustcall" : "hexrays_deregister_python_clearable_instance",
        },
    ...
    for chk in functions_coherence.values():
        chk["spotted"] = False
    ...
    def look_for_stuff(stuff, is_funcall, look_for_presence=True):
        ...
        for bit in stuff:
            ...
            if look_for_presence:
                ...
                if found:
                    cnts = "\n".join(fdef.contents)
                    raise LookupError(
                        f"Did find unwanted '{bit_pat}'\nfrom function "
                        f"'{check_for}'\nin file {one_file}\n(lines: {cnts})"
                        )

    for one_file in args.cpp_input.split(","):
        verb(f"Handling file: '{one_file}'")
        ...
        if parser.text.find("""in output value of type '""int""'");""") > -1:
            raise LookupError("Director output value type reporting doesn't "
        ...
        for fname, fdef in functions.items():
            ...
            check_for = None
            for fname in functions_coherence:
                if funstart.find(fname + "(") > -1:
                    check_for = fname
                    break

            if check_for:
                verb(f"Checking function at line {fdef.line_nr}:"
                    f" '{check_for}'")
                ...
                if "nostring" in chk:
                    look_for_stuff(chk["nostring"], False,
                                   look_for_presence=False)
                if "nullptrcheck" in chk:
                    mthd = fname.replace("_wrap_", "")\
                        .replace("__varargs__", "")
                    arg_ = chk["nullptrcheck"]
                    look_for_stuff(
                        f'"invalid null pointer " "in method \'" "{mthd}" "\','
                        f' argument " "{arg_}""', False)

    # Ensure all functions were spotted.
    for fname, chk in functions_coherence.items():
        if not chk["spotted"]:
            raise LookupError(f"Couldn't spot function '{fname}'")
...
def check_python(args):
    ...
    for chk in types_coherence.values():
        chk["spotted"] = False
                ...
                if tname in types_coherence:
                    ...
                    if "mustinherit" in tc:
                        if tc["mustinherit"] != parent:
                            must = tc["mustinherit"]
                            raise ValueError(
                                f"Type '{tname}' should inherit from '{must}'"
                                f" (and not '{parent}')"
                            )
            else:
                match = var_re.match(line)
                if match:
                    tc = types_coherence.get(match.group(1))
                    if tc:
                        tc["spotted"] = True


    # Ensure all types were spotted.
    for tname, chk in types_coherence.items():
        if not chk["spotted"]:
            raise LookupError(f"Couldn't spot type '{tname}'")



if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description='Check the generated idaapi_include.cpp file')
    ...
    if not pargs.cpp_input or not pargs.python_input:
        p.print_help()
        sys.exit(1)
