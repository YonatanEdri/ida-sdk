    ...
    def __render_docstr(self, doc, name):
        ...
        for l in doc.splitlines():
            ...
            if len(l) == 0:
                ...
                continue
            elif m is not None or "This method has the following signatures:" in l:
                # prototype definition
                continue
            else:
                # Pretty print ":param <varname>:" lines
                m = re.match(r":param ([a-zA-Z0-9+_-]+)[:\s].*", l)
                if m:
                    colored_name = ha( [ f"{m.group(1)}" ], [ il.SCOLOR_LOCNAME ])
                    l = colored_name + l[len(f":param {m.group(1)}"):]
                # Pretty print ":returns:" lines
                if l.startswith(":returns"):
                    l = l.replace(":returns", ha( [ "return" ], [ il.SCOLOR_ASMDIR ]))
                # if l.startswith(":return"):
                #     l = l.replace(":return", ha( [ "return" ], [ il.SCOLOR_ASMDIR ]))
