...
#define IDAPYTHON_USE_PYQT5_SHIM_VAR              "IDAPYTHON_USE_PYQT5_SHIM"
...
struct execution_t
{
  ...
  std::unique_ptr<newref_t> prev_trace_obj;   //lint !e958 padding of 5 bytes needed to align member on a 8 byte boundary
...
void execution_t::push()
{
  if ( entries.empty() )
  {
    ...
    if ( !idapython_plugin_t::get_instance()->owning_interpreter )
    {
      // If not owning interpreter, save the previous trace object in order
      // to be able to restore it later, otherwise a python debugger debugging
...
void execution_t::stop_tracking()
{
  if ( tracing_enabled )
  {
    ...
    {
      get_plugin_instance()->extapi.PyEval_SetTrace_ptr(nullptr, nullptr);
    }
  }
...
static const cfgopt_t opts[] =
{
  ...
  CFGOPT_N(IDAPYTHON_USE_PYQT5_SHIM_VAR, idapython_plugin_config_t, use_pyqt5_shim),
};
...
bool idapython_plugin_t::init()
{
  ...
  if ( !qgetenv(IDAPYTHON_USE_PYQT5_SHIM_VAR, nullptr) )
    qsetenv(IDAPYTHON_USE_PYQT5_SHIM_VAR,
      config.use_pyqt5_shim < 0    ? ""
      : config.use_pyqt5_shim == 0 ? "0"
      :                              "1");

  // Force QtPy to use pyside6 by default
  if ( !qgetenv("QT_API", nullptr) )
    qsetenv("QT_API", "pyside6");
...
static PyObject *my_CompileString(
        ...
        int start)
{
  return get_plugin_instance()->extapi.Py_CompileStringExFlags_ptr(str, file, start, nullptr, -1);
}
