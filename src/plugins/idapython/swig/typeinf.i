...
// get rid of this one, without prohibiting tinfo_t::set_named_type()
...
%ignore stkarg_area_info_t::get_info;
...
%ignore custom_callcnv_t::cbsize;
%ignore register_custom_callcnv;
%rename (register_custom_callcnv) py_register_custom_callcnv;
%ignore unregister_custom_callcnv;
%rename (unregister_custom_callcnv) py_unregister_custom_callcnv;

%director_method_result_to_bool_and_errbuf(validate_func, reterr);
%apply qstring *bout { qstring *outbuf };

// matches: py_register_custom_callcnv
%typemap(argout) custom_callcnv_t *cnv_incref {
  // %typemap(argout) custom_callcnv_t *cnv_incref
  if ( result != CM_CC_INVALID )
  {
    auto *p = new custom_callcnv_tuple_t;
    p->first = $1;
    p->second = borref_t($input); // Kernel gained a reference to the object
    kernel_registered_py_custom_callcnvs.push_back(p);
  }
}

// matches: py_unregister_custom_callcnv
%typemap(argout) custom_callcnv_t *cnv_decref {
  // %typemap(argout) custom_callcnv_t *cnv_decref
  if ( result )
  {
    auto *p = find_kernel_registered_py_custom_callcnv($1);
    if ( p != nullptr )
    {
      p->second = borref_t(nullptr); // Kernel lost reference to the object
      kernel_registered_py_custom_callcnvs.del(p);
    }
  }
}

%typemap(out) custom_callcnv_t * {
  // %typemap(out) custom_callcnv_t *
  if ( result != nullptr )
  {
    auto *p = find_kernel_registered_py_custom_callcnv($1);
    ...
    {
      $result = SWIG_NewPointerObj(SWIG_as_voidptr(result), SWIGTYPE_p_custom_callcnv_t, 0 |  0);
    }
  }
}
